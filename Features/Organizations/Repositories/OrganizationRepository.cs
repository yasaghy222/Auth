...
namespace Auth.Features.Organizations.Repositories
{
        ...
        IOrganizationRepository
    {
        ...
        {
            return await _db.Organizations.Where(i => i.Id == id && i.Status == OrganizationStatus.Active)
                .Include(i => i.Chides.Where(j => j.Status == OrganizationStatus.Active))
                .ThenInclude(i => i.Chides.Where(j => j.Status == OrganizationStatus.Active))
                .Select(i => i.MapToInfo()).FirstOrDefaultAsync(ct);
        }
    }
}